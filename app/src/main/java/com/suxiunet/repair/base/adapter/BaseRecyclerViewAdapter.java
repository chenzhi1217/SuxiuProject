 ...
 * author : chenzhi
 * time   : 2017/12/06
 * desc   : 所有RecyclerView的功用Adapter
 */
public abstract class BaseRecyclerViewAdapter<D> extends RecyclerView.Adapter<BasicHolder<D,? extends ViewDataBinding>> {
...
//            }
    ...
    @Override
    public BasicHolder<D,? extends ViewDataBinding> onCreateViewHolder(ViewGroup parent, int viewType) {
        if (checkLoadMoreAvailable() && viewType == mLoadMoreManager.getCurStateLayoutId()) {
            ...
            BasicHolder loadMoreViewHolder = new LoadMoreViewHolder(loadMoreView);
        ...
        final BasicHolder<? extends D,? extends ViewDataBinding> holder = onCreateItemHolder(parent, viewType);
//        holder.itemView.setBackgroundResource(R.drawable.selector_list_item_bg);
        return (BasicHolder<D,? extends ViewDataBinding>) holder;
    }

    protected abstract BasicHolder<? extends D,? extends ViewDataBinding> onCreateItemHolder(ViewGroup parent, int viewType);
    
    @Override
    public void onBindViewHolder(BasicHolder<D,? extends ViewDataBinding> holder, int position) {
    ...
    @Override
    public void onViewRecycled(BasicHolder<D,? extends ViewDataBinding> holder) {
        if (holder != null) {
